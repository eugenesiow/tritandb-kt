 ...
 */
class DecompressorTree(fileName:String) {
    private var input:BufferReader = BufferReader(ByteBuffer.allocate(1))
    ...
    fun readRows():Iterator<Row> = buildIterator {
        ...
        map.forEach({
            endOfStream = false
            blockTimestamp = it.key
            storedTimestamp = -1L
            val bb = ByteBuffer.wrap(it.value)
            input = BufferReader(bb)
